...
let rec eval_expr env e = 
  ...
  | FunctionCall(expr1, expr2) -> (
    ...
    | Closure(envir, str, expr1) -> (
      match (eval_expr envir expr2) with 
      | Int(x) -> eval_expr (extend envir str (Int(x))) expr1
      | Bool(x) -> eval_expr (extend envir str (Bool(x))) expr1
      | String(x) -> eval_expr (extend envir str (String(x))) expr1
      | Closure(a, b, c) -> eval_expr (extend envir str (Closure(a, b, c))) expr1
    )
